...
Map<String, AchievementModel> allAchievements = {
  ...
  "Subject Master": AchievementModel(
      ...
      achievementChecker: (quizResults) {
        //  Get a list of average scores from each quiz category
        ...
        //  Check if all compiled average scores have met the quota
  ...
  "Challenge Conqueror": AchievementModel(
      ...
      achievementChecker: (quizResults) {
        //  Get all the difficult quiz scores and store them in a List
        ...
        //  Check if there is a quiz score less than 8
...
AchievementModel? getAchievement(String achievementName) {
  for (int i = 0; i < allAchievements.length; i++) {
    if (allAchievements.keys.toList()[i] == achievementName) {
...
double getAverageSpeechConfidence(List<dynamic> confidenceLevels) {
  ...
  if (count == 0) {
    return 0; // Return 0 if no valid double values are found
  }
